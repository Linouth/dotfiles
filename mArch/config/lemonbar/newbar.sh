...
ip() {
	ipaddr=$(ip=$(ifconfig | grep enp3s0 -A 1 | grep -oE '((1?[0-9][0-9]?|2[0-4][0-9]|25[0-5])\.){3}(1?[0-9][0-9]?|2[0-4][0-9]|25[0-5])' | awk '(NR<2)'); if [ ! -z "$ip" ]; then echo "$ip"; else echo "No Connection";fi)
...
while :; do
	...
	sleep .5
done | lemonbar -g ${PW}x${PH}+${PX}+${PY} -f "$FONT2" -f "$FONT1" -B "$BG" -F "$FG" -p -d | while :; do read line; eval $line; done &
